  ...
  >
    ...
    <template v-else>
      ...
      <b>
        ...
        <span v-if="showIPRate"> 무한 포인트</span>
        <span v-else> 무한 {{ pluralize("포인트", gainedIP) }}</span>
      </b>
      <template v-if="showIPRate">
        <br>
        현재: {{ format(currentIPRate, 2) }} 무한 포인트/분
        <br>
        최고: {{ format(peakIPRate, 2) }} 무한 포인트/분
        <br>
        {{ format(peakIPRateVal, 2) }} 무한 포인트에서
      </template>
      <div v-else />
    </template>
  </button>
