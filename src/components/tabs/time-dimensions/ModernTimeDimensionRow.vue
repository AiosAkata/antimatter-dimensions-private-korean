...
export default {
  ...
  computed: {
    ...
    tooltipContents() {
      ...
      if (this.isCapped) return `Nameless가 ${format(1)}개 이상의 시간 차원 구매를 방지합니다`;
      return `${quantifyInt("회", this.bought)} 구매됨`;
    },
    showRow() {
    ...
    formattedEPCost() {
      return this.isCapped ? "한계치" : `${this.showCostTitle ? "비용: " : ""}${format(this.cost, 2)} EP`;
    },
    hasLongText() {
    ...
    timeEstimate() {
      ...
      return time.gt(0) ? `${TimeSpan.fromSeconds(time.toNumber()).toStringShort()} 후에 충분한 TT 획득` : "";
    }
  },
  watch: {
