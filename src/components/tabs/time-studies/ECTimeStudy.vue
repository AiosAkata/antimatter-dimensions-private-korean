...
<template>
  <TimeStudyButton :setup="setup">
    ...
    <template v-if="hasRequirement">
      ...
      <span v-if="config.secondary.path">{{ config.secondary.path }} 경로만 사용합니다</span>
      <span v-else>
    ...
    <span v-if="isUnlocked && !isRunning"><br>시작하려면 두 번 클릭하세요</span>
    <span v-else-if="isRunning"><br>현재 실행 중</span>
  </TimeStudyButton>
</template>
