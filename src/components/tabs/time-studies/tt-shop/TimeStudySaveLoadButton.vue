...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      const presetName = this.name ? `시간 연구 프리셋 "${this.name}"` : "시간 연구 프리셋";
      GameUI.notify.eternity(`${presetName}이(가) 슬롯 ${this.saveslot}에 저장되었습니다`);
    },
    load() {
      this.hideContextMenu();
      if (this.preset.studies) {
        ...
        const presetName = this.name ? `시간 연구 프리셋 "${this.name}"` : "시간 연구 프리셋";
        GameUI.notify.eternity(`${presetName}이(가) 슬롯 ${this.saveslot}에서 로드되었습니다`);
      } else {
        Modal.message.show("이 시간 연구 목록에는 현재 시간 연구가 없습니다.");
      }
    },
    respecAndLoad() {
    ...
    deletePreset() {
      ...
      else Modal.message.show("이 시간 연구 목록에는 현재 시간 연구가 없습니다.");
    },
    handleExport() {
      this.hideContextMenu();
      copyToClipboard(this.preset.studies);
      const presetName = this.name ? `시간 연구 프리셋 "${this.name}"` : "시간 연구 프리셋";
      GameUI.notify.eternity(`${presetName}이(가) 슬롯 ${this.saveslot}에서 클립보드로 내보내졌습니다`);
    },
    edit() {
  ...
  <HoverMenu class="l-tt-save-load-btn__wrapper">
    ...
    <template #menu>
      <div class="l-tt-save-load-btn__menu c-tt-save-load-btn__menu">
        <span ach-tooltip="사용자 정의 이름 설정 (최대 4개의 ASCII 문자)">
          <input
            type="text"
            size="4"
            maxlength="4"
