...
export default {
  ...
  computed: {
    ...
    errorTooltip() {
      return `스크립트에 ${quantify("error", this.errorCount)}이(가) 있습니다`;
    },
    nameTooltip() {
      return this.isNameTooLong
        ? `이름은 ${formatInt(AutomatorData.MAX_ALLOWED_SCRIPT_NAME_LENGTH)}글자 이하여야 합니다!`
        : "";
    },
    currentScriptID: {
      get() {
    ...
    importTooltip() {
      return this.canMakeNewScript
        ...
        : "스크립트가 너무 많아서 다른 스크립트를 가져올 수 없습니다!";
    },
  ...
  methods: {
    ...
    exportScript() {
      ...
      if (toExport) {
        ...
        GameUI.notify.error("빈 오토메이터 스크립트를 내보낼 수 없습니다!");
      }
    },
    importScript() {
    ...
    updateCurrentScriptID() {
      ...
      if (this.isBlock && BlockAutomator.hasUnparsableCommands(this.currentScript) && this.currentScript !== "") {
        ...
        Modal.message.show("일부 스크립트 명령을 인식할 수 없습니다 - 텍스트 편집기로 전환합니다.");
      }
