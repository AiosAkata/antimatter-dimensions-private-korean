...
export default {
  ...
  computed: {
    ...
    statusText() {
      ...
      if (this.isPaused) return `일시정지됨: "${this.statusName}" (${lineNum}번 줄에서 재개)`;
      if (this.isRunning) return `실행 중: "${this.statusName}" (${lineNum}번 줄)`;
      if (this.hasErrors) return `정지됨: "${this.statusName}"에 오류 있음 (실행 불가)`;
      return `정지됨: "${this.statusName}" 실행 예정`;
    },
    maxScriptChars() {
  ...
  <div class="c-automator__controls l-automator__controls">
    <div class="c-automator-control-row l-automator-button-row">
      <div class="c-button-group">
        <AutomatorButton
          v-tooltip="'오토메이터를 첫 번째 명령으로 되돌리기'"
        ...
        />
        <AutomatorButton
          v-tooltip="'오토메이터 정지 및 위치 초기화'"
          class="fa-stop"
          @click="stop"
        />
        <AutomatorButton
          v-tooltip="'한 줄 앞으로 이동'"
          class="fa-step-forward"
          @click="step"
        />
        <AutomatorButton
          v-tooltip="'스크립트가 끝에 도달할 때 자동으로 재시작'"
          class="fa-sync-alt"
          :class="{ 'c-automator__button--active' : repeatOn }"
          @click="repeat"
        />
        <AutomatorButton
          v-tooltip="'현실 완료 또는 재시작 시 활성 스크립트 자동 재시작'"
          class="fa-reply"
    ...
    <div class="l-automator-button-row">
      <span
        ...
        v-tooltip="'같은 이름의 스크립트가 두 개 이상입니다!'"
