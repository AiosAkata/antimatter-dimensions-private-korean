...
export default {
  ...
  methods: {
    ...
    currentError() {
      ...
      if (!isValidName) return "상수 이름은 공백 없이 영숫자여야 하며 숫자로 시작할 수 없습니다";
      if (alreadyExists) return "이미 이 이름의 상수를 정의했습니다";
      if (hasCommandConflict) return "상수 이름이 명령 키워드와 충돌합니다";
      if (shadowsPrototype) return "상수 이름은 내장 Javascript 프로토타입 속성을 숨길 수 없습니다";

      if (!this.valueString) return "상수 값은 비워둘 수 없습니다";
      ...
      if (!isNumber && !isStudyString) return "상수 값은 숫자 또는 시간 연구 문자열이어야 합니다";
      return null;
    },
    errorTooltip() {
